...
def rebalance_process(): #ขั้นตอนในการ Reblalnce 
    ...
    if diffchange >= float(global_config_val['percent']) : #ด้านที่ราคาสินทรัพย์ที่ถือขึ้นเราต้องนำไปขาย 
        ...
        with open('config.ini', 'w') as f:
          config.write(f)    
        signal_buy_txt = (f'Buy : {signal_buy_reverse} Bath')
        signal_txt = signal_buy_txt
        ...
        with open('config.ini', 'w') as f:
            config.write(f)
        signal_wait = ' Wait Next time '
        print (signal_wait)
        signal_txt = signal_wait
        #messenger.sendtext(' Wait Next time ')
    diff_txt = (f'Diff : {diffchange}')
    return diff_txt , signal_txt
...
Showtrade_sym = global_config_val['trade_sym']
Showpercent = global_config_val['percent']

resulttradesym_txt = (f'Your Rebalance :{Showtrade_sym}')
resultDiff_txt = (f'Diff Rebalance :{Showpercent}%')
